...
def main(raw_args=None):
    ...
    if IN_SHORT != '':
        ...
        if exists_and_is_nonZero(IN_BWALOG) is False:
    ...
    VIRAL_JSON = args.v1
    if exists_and_is_nonZero(VIRAL_JSON) is False:
    ...
    if exists_and_is_nonZero(VIRAL_ACCESSIONS) is False:
    ...
    if len(IN_SHORT):
        ...
        for line in f:
            ...
            data_byReadName[myName].append([myRef, myPos, myCig, myMapQ])
            # alternate cigar
            if ind_altc is not None and splt[ind_altc] != '-':
        ...
        for k in data_byReadName.keys():
            ...
            if len(data_byReadName[k]) == 2:
                ...
                # human - virus only
                    ...
                    found_a_hit = False
                    for i in range(len(clustered_events)):
                        for j in range(len(clustered_events[i])):
    ...
    for i in range(len(clustered_events)):
        ...
        if isInBadRange(r1,pmin) is False and isInBadRange(r1,pmax) is False:
    ...
    if len(INPUT_BAM) and exists_and_is_nonZero(INPUT_BAM) and len(IN_SHORT):
        ...
        for k in sc_track.all_tracks.keys():
            for i in range(1, len(sc_track.all_tracks[k])-2, 2):
    ...
    for i in order_to_process_clusters:
        ...
        if i < len(evidence_sc):
            ...
            for n in evidence_pe[i]:
                for j in list(range(n[0],n[1]+1)) + list(range(n[2],n[3]+1)):
        ...
        if not len(sc_count):
            ...
            if (len(args.c) or len(COMPARE)) and len(scl) >= MIN_SOFTCLIP:
                ...
                if sc_anyHits is False and len(sc_fps):
        ...
        if not len(pe_count):
            ...
            threshes = [n*(max_pe/POLY_PE_STEPS) for n in range(POLY_PE_STEPS)] + [max_pe-1]
            myPECount = max_pe
            ...
            pe_to_report_stratified = [[pe_to_report[n] for n in range(len(pe_to_report)) if pe_density[n] == m] for m in range(0,POLY_PE_STEPS+1)]
            ...
            if (len(args.c) or len(COMPARE)) and myPECount >= MIN_DISC_ONLY:
                pe_coords_to_compare = sorted_coord_list_to_ranges([pe_to_report[n] for n in range(len(pe_to_report)) if pe_density[n] >= POLY_PE_STEPS-1])
        ...
        if SKIP_PLOTTING is False:
            ...
            if max_pe:
                ...
                for j in range(len(poly_pe_stratified)):
        ...
        myClass = (1*(sc_to_report is not None and len(sc_to_report) > 0), 1*(pe_to_report is not None and len(pe_to_report) > 0), 1*('CCS' in pbtypes), 1*('CLR' in pbtypes))
        ...
        #
        bed_out = []
        if SKIP_HG38_FILT is False:
            for bed_i in range(len(BED_TRACKS)):
        ...
        if max_sc and len(evidence_pb[i]):
            first_clr = True
            first_ccs = True
            for j in range(len(sc_to_report)):
                for k in range(len(evidence_pb[i])):
    ...
    if len(args.c) or len(COMPARE):
        ...
        for n in COMPARE:
            if len(COMPARE_OUT[n]) == 0:
                ...
                min_distance_co_item = sorted([(COMPARE_OUT[n][i][4], i) for i in range(len(COMPARE_OUT[n]))])[0][1]
