...
def condenseListOfRegions(l):
    ...
    for i in range(1,len(l)):
        if l[i][1] <= prevR[1]:
            ...
            prevR = l[i]
    condensedInput = [l[i] for i in range(len(l)) if delList[i] is False]
    ...
    delList.reverse()
    return [condensedInput[i] for i in range(len(condensedInput)) if delList[i] is False]
